...
from core import db
from core.models.assignments import Assignment, AssignmentStateEnum, GradeEnum, AssignmentStateEnum
...
def test_get_assignments_in_graded_state_for_each_student():
    ...
    db.session.flush()
    db.session.commit()
    db.session.expunge_all()  # Optional but useful to clear cached instances
    ...
    print("graded_assignments", graded_assignments)
    ...
    # assert sql_result == expected_result, f"Expected: {expected_result}, Got: {sql_result}"
